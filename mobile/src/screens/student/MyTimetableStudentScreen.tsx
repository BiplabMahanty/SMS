...
import { fetchMyTimetableStudent } from '../../store/slices/timetableSlice';

export const MyTimetableStudentScreen: React.FC = () => {
  const dispatch = useAppDispatch();
  const navigation = useNavigation();
  const { myEntries, loading, error } = useAppSelector((s) => s.timetable);

  const load = () => { dispatch(fetchMyTimetableStudent({})); };
