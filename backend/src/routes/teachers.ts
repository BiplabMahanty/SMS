...
import { validateCreateTeacher, validateUpdateTeacher } from '../validators/teacherValidator';
...
// Teacher self-service routes (must come before /:id to avoid conflict)
...
// POST /api/teachers/:id/assign-class  — assign a class+section to a teacher
router.post('/:id/assign-class', authorize('ADMIN'), async (req: Request, res: Response, next: NextFunction) => {
  try {
...
// DELETE /api/teachers/:id/assign-class  — remove a class assignment from a teacher
router.delete('/:id/assign-class', authorize('ADMIN'), async (req: Request, res: Response, next: NextFunction) => {
  try {
    const { classId, sectionId, academicYear } = req.body;
    if (!classId || !academicYear) throw new AppError('classId and academicYear are required', 400);
    if (!mongoose.Types.ObjectId.isValid(req.params.id)) throw new AppError('Invalid teacher ID', 400);
